...
app.render("/", function (req, res) {
    //we call the function to check in ours views folder for the index hbs
    res.render("index", {
        layout: false, //handlebars express turn off layout
        name: "User", //Username specified in the hbs index
        age: Math.floor(Math.random() * 100), //Random output of number(age) specified in the hbs index
    }); //Render by view engine the hbs html (by the index hbs file)(handlebars express turn off layout)(specified name shown in the name tab)
});
app.get("/greeting", function (req, res) {
    //we define action - answers requests
    res.send("Hey there user"); //once request is called the following answer will be received
});
app.get("/ask", (req, res) => {
    //action definiton with arrow function
